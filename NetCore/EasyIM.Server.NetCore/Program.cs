...
namespace EasyIM.Server.NetCore
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                ResourceCenter<IMUser, IMGroup> resourceCenter = new ResourceCenter<IMUser, IMGroup>(rapidServerEngine, persister, new TalkBaseHelper(), infoTypes, logger);
                ...
                /*
                 * 注意：讨论组或群，是通过ESPlus.Application.Contacts命名空间来实现的。
                 * 
                ...
                Console.WriteLine("EasyIM Service Started! listening on port :" + ConfigurationManager.AppSettings["Port"]);
            }
            catch (Exception ee)
            {
                logger.Log(ee, "Main", ErrorLevel.High);
                Console.WriteLine("启动EasyIM Server 失败：" + ee.Message);
                Console.ReadLine();
            }
