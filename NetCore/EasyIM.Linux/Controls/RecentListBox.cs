...
namespace EasyIM.Linux.Controls
{
    ...
    internal class RecentListBox : Control
    {
        ...
        {
            //加载最近联系人
            DefaultLastWordsComputer<IMUser, IMGroup> computer = new DefaultLastWordsComputer<IMUser, IMGroup>(Program.ResourceCenter);
            int insertIndex = 0;
        ...
        private RecentContactModel ConvertToRecentContactModel(IUnit unit)
        {
            ...
            if (unit.IsUser)
            {
                return new RecentContactModel((IMUser)unit);
            }
            else if (unit.UnitType == TalkBase.UnitType.Group)
            {
                return new RecentContactModel((IMGroup)unit);
            }
            return null;
        }
