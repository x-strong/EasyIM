...
using EasyIM.Controls.ChatRender4Dll;
using EasyIM.Linux.Controller;
using EasyIM.Linux.Controls.ChatRender4Dll;
...
namespace EasyIM.Linux.Controls
{
    internal class ChatPanel : Control, IUserNameGetter, OrayChatContextMenuHandler
    {
        ...
        { 
            ...
            {
                this.AppendChatBoxContent(isMe, isMe ? Program.ResourceCenter.ClientGlobalCache.CurrentUser : (IMUser)this.destUnit, this.destUnit.LastWordsRecord.SpeakTime, content);
            }
            else
            {
                ...
                this.AppendChatBoxContent(isMe, (IMUser)speaker, this.destUnit.LastWordsRecord.SpeakTime, content);
            }
        }
        ...
        private void SetHistoryChatMessageRecord(ChatRecordPage page, bool lastPage)
        {
            ...
            for (int i = 0; i < leftCount; i++)
            {
                ...
                if (this.destUnit.IsUser)
                {
                    ...
                    this.AppendChatBoxContent(isMe, isMe ? Program.ResourceCenter.ClientGlobalCache.CurrentUser : (IMUser)this.destUnit, record.OccureTime, content);
                }
                else
                {
                    string talker = speaker.DisplayName + "（" + record.SpeakerID + ")";  //string.Format("{0}({1})", speaker.DisplayName, record.SpeakerID);
                    this.AppendChatBoxContent(isMe, (IMUser)speaker, record.OccureTime, content);
                }
        ...
        public void AppendChatBoxContent(bool isMe, IMUser sender, DateTime originTime, ChatBoxContent2 content)
        {
        ...
        {
            IMInputContent inputContent = this.inputbox.GetInputContent();
            ChatBoxContent2 chatBoxContent = new ChatBoxContent2(inputContent.Text, SystemSettings.DefaultSimpleFont, System.Drawing.Color.Black);
            foreach (KeyValuePair<int, int> item in inputContent.EmotionDictionary)
            {
                chatBoxContent.AddEmotion((uint)item.Key, (uint)item.Value);
            }
            foreach (KeyValuePair<int, Image> item in inputContent.ImageDictionary)
            {
        ...
        public void HandleChatMessage(string broadcasterID, ChatBoxContent2 content, DateTime? originTime, string tag)
        {
            ...
            if (this.destUnit.IsUser)
            {
                ...
                //    followingWords = (DateTime.Now - this.destUnit.LastWordsRecord.SpeakTime).TotalSeconds <= 30;
                //}
                this.AppendChatBoxContent(false, (IMUser)this.destUnit, originTime.GetValueOrDefault(), content);
            }
            else
            {
                IMUser user = Program.ResourceCenter.ClientGlobalCache.GetUser(broadcasterID);
