...
namespace EasyIM.Linux.Controls
{
    ...
    internal class FriendListBox : Control
    {
        ...
        private Comparison<IMUserPlus> comparison4User;
        ...
        public void Initialize()
        {
            ...
            this.comparison4User = new Comparison<IMUserPlus>(this.Comparison4UserItem);
        ...
        #region 好友相关
        public void AddUser(IMUserPlus user,bool needRefreshSource = false)
        {
            lock (this.locker)
            {
                Collection<IMUserPlus> userList = this.GetCatalogUserList4User(user);
                if (userList == null) { return; }
                foreach (IMUserPlus item in userList)
                {
                    if (item.ID == user.ID)
                    {
                        return;
        ...
        {
            IMUser user = Program.ResourceCenter.ClientGlobalCache.GetUser(userID);
            this.AddUser(user);
        }

        private void AddUser(IMUser user)
        {
            lock (this.locker)
            {
                if (user == null) { return; }
                this.AddUser(new IMUserPlus(user),true);
            }
        }
        ...
        public void UnitInfoChanged(IUnit unit)
        {
            lock (this.locker)
            {
                Collection<IMUserPlus> userInList = null;
                IMUser user = (IMUser)unit;
                foreach (FriendListModel model in models)
                {
                    for (int i = 0; i < model.UserList.Count; i++)
                    {
                        ...
                        {
                            //ESBasic.Helpers.ReflectionHelper.CopyProperty(new IMUserPlus(user), model.UserList[i]);
        ...
        private Collection<IMUserPlus> GetCatalogUserList4User(IUser user)
        {
        ...
        public void AddCatalog(string catelogName, bool needNotice)
        {
            ...
            this.models.Add(new FriendListModel() { CatalogName = catelogName, UserList = new Collection<IMUserPlus>() });
            models.Sort(this.comparison4ListItem);
            if (needNotice)
            {
        ...
        {//模板定义
            ...
            Children.Add(
                    ...
                    { 
                        ...
                        ItemTemplate = new EasyIM.Linux.Controls.CatalogItem
                        {
                            ItemTemplate = new UserItem()
                            {
                                Commands ={
                                {
                                    nameof(MouseDown),(s,e)=>{
                                        ...
                                        {
                                            args.Handled = true;
                                            IMUserPlus user = s.DataContext as IMUserPlus;
                                            if(user==null){ return; }
                                            if(user.ID==Program.ResourceCenter.CurrentUserID){ return; }
        ...
        public int Comparison4UserItem(IMUserPlus x, IMUserPlus y)
        {
            if (x == y)
            {
                return 0;
        ...
        public void ChangeCatelogName(string oldName, string newName)
        {
            ...
            if (this.ExistsFriendListModel(newName))
            {
                ...
                Collection<IMUserPlus> userList = this.GetFriendListModel(newName).UserList;
        ...
        public void Catalog4Moved(string friendID, string oldCatalogName, string newCatalogName)
        {
            lock (this.locker)
            {
                ...
                IMUserPlus frined = friendListModel_old.UserList.Find(o => o.ID == friendID);
        ...
        {
            IMUserPlus user = this.treeView.SelectedValue as IMUserPlus;
            if (user == null) { return; }
            try
            {
                if (Program.ResourceCenter.ClientGlobalCache.CurrentUser.UserStatus == UserStatus.OffLine)
                ...
                if (user.ID == Program.ResourceCenter.RapidPassiveEngine.CurrentUserID)
                {
                    return;
                }
                Task<ButtonResult> task = MessageBoxEx.ShowQuery(string.Format("您确定要删除好友 {0}({1}) 吗？", user.Name, user.ID));
                await task.ConfigureAwait(true);
                ...
                Program.ResourceCenter.ClientOutter.RemoveFriend(user.ID);
                GlobalResourceManager.Logger.LogWithTime("发送删除好友消息执行完成");
                this.DeleteUser(user.ID);
                MainWindow.ChatFormController.CloseForm(user.ID);
            }
            catch (Exception ee)
            {
        ...
        private async void MoveToCatalog_Click(CpfObject s)
        {
            ...
            if (user == null || user.ID == Program.ResourceCenter.CurrentUserID) { return; }
            string catelogName = Program.ResourceCenter.ClientGlobalCache.CurrentUser.GetFriendCatalog(user.ID);
            ...
            if (Convert.ToBoolean(task.Result))
            {
                Program.ResourceCenter.ClientOutter.MoveFriendCatalog(user.ID, catelogName, friendCatelogSelectorWindow.NewName);
                this.Catalog4Moved(user.ID, catelogName, friendCatelogSelectorWindow.NewName);
        ...
        private async void UpdateFirendRemark_Click(CpfObject s)
        {
            ...
            if (user == null) { return; }
            if (user.ID == Program.ResourceCenter.CurrentUserID) { return; }
            EditCommentNameWindow form = new EditCommentNameWindow(user.CommentName);
            System.Threading.Tasks.Task<object> task = form.ShowDialog_Topmost((Window)this.Root);
            await task.ConfigureAwait(true);

            if (Convert.ToBoolean(task.Result))
            {
                Program.ResourceCenter.ClientOutter.ChangeUnitCommentName(user.ID, form.NewName);
                user.CommentName = form.NewName;
            }
        }
        ...
        private void MessageRecord_Click(CpfObject s)
        {
            IMUserPlus user = this.treeView.SelectedValue as IMUserPlus;
            if (user == null) { return; }
            Window form = MainWindow.ChatFormController.GetChatRecordForm(user.ID);
            if (form != null)
            {
                form.Show_Topmost();
            }
