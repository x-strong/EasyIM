...
namespace EasyIM.Linux.Controls
{
    ...
    {       
        ...
        private void TreeView_DoubleTapped(CpfObject sender)
        {
            ...
            if (sourceData is RecentContactModel)
            {
                ...
                try
                {
                    ...
                    if (model.IsUser)
                    {
                        IMUser friend = Program.ResourceCenter.ClientGlobalCache.GetUser(model.ID);
                        this.StartUserChat(friend);
                    }
                    else
                    {
                        IMGroup group = Program.ResourceCenter.ClientGlobalCache.GetGroup(model.ID);
                        this.StartGroupChat(group);
                    }
                }
                catch (Exception ee)
                {
        ...
        private async void StartGroupChat(IMGroup group)
        {
            if (group == null)
            {
                return;
        ...
        private void TextBox_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            List<IMUser> users2 = Program.ResourceCenter.ClientGlobalCache.SearchUser_Fuzzy(idOrName);
            List<IMGroup> groups2 = Program.ResourceCenter.ClientGlobalCache.SearchGroup_Fuzzy(idOrName);
            this.tb_noResult.Text = "按回车获取结果";
            List<IGroup> iGroups = new List<IGroup>();
            if (groups2 != null)
            {
                iGroups = ESBasic.Collections.CollectionConverter.ConvertListUpper<IGroup, IMGroup>(groups2);
            }
            this.SetSearchResult(users2, iGroups);
        }
        ...
        private void SetSearchResult(List<IMUser> users, List<IGroup> groups, bool isFromServer = false)
        {
            ...
            if (!hasNoResult)
            {
                foreach (IUser unit in users)
                {
                    ...
                    RecentContactModel model = new RecentContactModel((IMUser)unit);
                ...
                foreach (IGroup group in groups)
                {
                    ...
                    RecentContactModel model = new RecentContactModel((IMGroup)group);
                    model.CommentName = describe;
