...
namespace EasyIM.Linux.Controls
{
    internal class NDiskBrowser : Control, INDiskBrowser
    {
        ...
        public NDiskBrowser()
        {
            this.imageList2.Add(EasyIM.Linux.Helpers.FileHelper.FindAssetsBitmap("tick.png"));
            this.imageList2.Add(EasyIM.Linux.Helpers.FileHelper.FindAssetsBitmap("error.png"));
        ...
        private async void ToolStripButton_uploadFile_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                Task<string[]> task = EasyIM.Linux.Helpers.FileHelper.OpenFileDialog(this.parentWindow, "请选择要上传的文件", false);
        ...
        private void GotoParent()
        {
            try
            {
                ...
                if (directoryInfo.Parent == null || !temp.Contains(EasyIM.Linux.Helpers.FileHelper.FolderSeparatorChar))
                {
                    this.GotoRoot();
                    return;
                }
                int pos = temp.LastIndexOf(EasyIM.Linux.Helpers.FileHelper.FolderSeparatorChar);
                string relativeDir = temp.Substring(0, pos) + EasyIM.Linux.Helpers.FileHelper.FolderSeparatorChar;
        ...
        private async void Download(string fileOrDirName, bool isFile)
        {
            ...
            Task<string> task = EasyIM.Linux.Helpers.FileHelper.SaveFileDialog(this.parentWindow, tip, fileOrDirName);
            await task.ConfigureAwait(true);
        ...
        private void LoadDirectory(string path, bool tipOnException)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (this.currentDirPath != null && !this.currentDirPath.EndsWith(EasyIM.Linux.Helpers.FileHelper.FolderSeparatorChar))
                    {
                        this.currentDirPath += EasyIM.Linux.Helpers.FileHelper.FolderSeparatorChar;
                    }
                    ...
                    {
                        displayPath += EasyIM.Linux.Helpers.FileHelper.FolderSeparatorChar + this.currentDirPath;
                    }
