...
namespace EasyIM.Linux.Views
{
    internal class MainWindow : Window, ITwinkleNotifySupporter
    {
        ...
        private Event2ChatFormBridge<IMUser, IMGroup> event2ChatFormBridge;
        private Event2ChatListBridge<IMUser, IMGroup> event2ChatListBridge;
        internal Event2ChatListBridge<IMUser, IMGroup> Event2ChatListBridge
        {
                                ...
                                { 
                                    ...
                                    Children =
                                    {
                                        ...
                                        { 
                                            ...
                                            Bindings={ { nameof(Picture.ToolTip),nameof(IMUserPlus.ID), null, BindingMode.OneWay, x => "账号 ：" + x  } }
                                        },
                                        new StackPanel
                                        {
                                            Size = SizeField.Fill,
                                            ...
                                            { 
                                                new TextBlock
                                                {
                                                    ...
                                                    MarginTop = 20,
                                                    Text = "傲瑞客服",
                                                    Bindings={ { nameof(TextBlock.Text),nameof(IMUserPlus.Name)} }
                                                },
                                                new StackPanel
                                                {
                                                    Width= "100%", 
                                                    ...
                                                    { 
                                                        new Panel
                                                        {
                                                            ...
                                                            Children =
                                                            {
                                                                new Picture
                                                                {
                                                                    ...
                                                                    Bindings={ { nameof(Picture.Source),nameof(IMUserPlus.UserStatus),null,BindingMode.OneWay, a => { return CommonHelper.GetUserStatusIco((UserStatus)a); } }},
                                                                    Commands={ { nameof(MouseDown),(s,e)=> {
                                                        ...
                                                        new TextBlock
                                                        {
                                                            ...
                                                            MaxWidth = 130,
                                                            TextTrimming = TextTrimming.CharacterEllipsis,
                                                            Bindings={ { nameof(TextBlock.Text),nameof(IMUserPlus.Signature)} }
                                                        },
                                                    }
                                                }
                                            }
        ...
        private void Initialize(UserStatus userStatus)
        {
            ...
            this.event2ChatFormBridge = new Event2ChatFormBridge<IMUser, IMGroup>(Program.ResourceCenter);
        ...
        {
            IMUserPlus userPlus = new IMUserPlus(Program.ResourceCenter.ClientGlobalCache.CurrentUser);
            this.DataContext = userPlus;
            this.headPic.Fill = new TextureFill() { Image = userPlus.HeadImage, Stretch = Stretch.Fill };
        }
        ...
        private void InitView()
        {
            ...
            this.event2ChatListBridge = new Event2ChatListBridge<IMUser, IMGroup>(Program.ResourceCenter, this.friendListBox, this.groupListBox, this.recentListBox);
        ...
        private void ClientGlobalCache_GroupChanged(IMGroup group, GroupChangedType type, string operatorID, string target)
        {
            if (type == GroupChangedType.GroupDeleted)
            {
                messageCacheManager.RemoveUnhanleMessage(group);
            }
        }

        private void ClientGlobalCache_UserDeleted(IMUser user)
        {
            messageCacheManager.RemoveUnhanleMessage(user);
        }
        ...
        private async void groupChatForm_ExitGroupClicked(string groupID)
        {
            try
            {
                ...
                IMGroup group = Program.ResourceCenter.ClientGlobalCache.GetGroup(groupID,true);
        ...
        private string GetNotifyMsgDescribe(string userID, int notifyType, byte[] info,out TalkBase.UnitType unitType)
        {
            ...
            else if (notifyType == Program.ResourceCenter.TalkBaseInfoTypes.RequestAddGroup)
            {
                ...
                IMGroup group = Program.ResourceCenter.ClientGlobalCache.GetGroup(contract.GroupID);
                msgDescribe = string.Format("{0}申请加入群{1}", userName, group.DisplayName);
            ...
            else if (notifyType == Program.ResourceCenter.TalkBaseInfoTypes.HandleAddGroupRequest)
            {
                ...
                IMGroup group = Program.ResourceCenter.ClientGlobalCache.GetGroup(contract.GroupID);
                msgDescribe = string.Format("{0}{1}您加入群{2}", userName, contract.IsAgreed ? "同意了" : "拒绝了", group.DisplayName);
                unitType = UnitType.Group;
            }
        ...
        void do_rapidPassiveEngine_ConnectionInterrupted()
        {
            ...
            foreach (IMUser friend in Program.ResourceCenter.ClientGlobalCache.GetAllUser())
            {
        ...
        {
            IMUser user = Program.ResourceCenter.ClientGlobalCache.GetUser(friendID);
            return user.Name;
        }

        public string GetGroupName(string groupID)
        {
            IMGroup group = Program.ResourceCenter.ClientGlobalCache.GetGroup(groupID);
            return group.Name;
        }
