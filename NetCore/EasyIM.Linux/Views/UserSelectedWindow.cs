...
namespace EasyIM.Linux.Views
{
    internal class UserSelectedWindow : BaseWindow
    {
        ...
        public void Initialize4Group(IMGroup group, bool _showMyself)
        {
        ...
        private void BaseUserListBox_User_DoubleTapped(IMUserPlus userPlus)
        {
            this.AddUserSelectedItem(userPlus);
        }
        ...
        private void DoSearch()
        {
            ...
            List<IMUser> users = Program.ResourceCenter.ClientGlobalCache.SearchUser(idOrName);
        ...
        private void AddUserSelectedItem(IMUser user)
        {
            if (user == null) { return; }
            if (this.userSelectedViewModel.UserSelectedModels.Exists(x => x.ID == user.ID))
            {
                return;
            }
            UserSelectedModel selectedModel = new UserSelectedModel(user.ID, user.Name, GlobalResourceManager.GetHeadImage(user));
