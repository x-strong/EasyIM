...
namespace TalkBase.Client.Bridges
{
    ...
    {      
        ...
        void do_clientOutter_MediaCommunicateReceived(string userID, ClientType clientType, CommunicateMediaType type, CommunicateType type2, string tag)
        {
            ...
            {
                if (string.IsNullOrEmpty(EasyIM.CommonOptions.CallingID4VideoOrVoice) || 
                    (!string.IsNullOrEmpty(EasyIM.CommonOptions.CallingID4VideoOrVoice) && EasyIM.CommonOptions.CallingID4VideoOrVoice != userID))
                {
        ...
        {
            EasyIM.CommonOptions.CallingID4VideoOrVoice = string.Empty;
            foreach (IChatForm form in this.resourceCenter.ChatFormController.GetAllForms())
            {
                form.MyselfOffline();
            }
