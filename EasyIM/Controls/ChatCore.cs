...
namespace EasyIM.Controls
{
    #region MessageBox
    //控件定义的接口
    internal interface IIMChatControl
    {
    ...
    //系统消息提示
    internal class IMSystemTipLabel : IIMChatControl
    {
        ...
        public IMSystemTipLabel(String SystemTipLabel, IChatControl _parent)
        {
    ...
    //发送表情和文本
    internal class IMChatContentEmotionAndText : IIMChatControl
    {

        private List<IIMChatInputItem> AllTestAndEmotion;
        ...
        public IMChatContentEmotionAndText(List<IIMChatInputItem> all, IChatControl chatBox, SenderInfo senderInfo)
        {
        ...
        private void Init(List<IIMChatInputItem> all, bool isme, IChatControl chatBox)
        {
            ...
            {
                IIMChatInputItem aloneItem = AllTestAndEmotion[i];
                IMChatInputText aloneText = aloneItem as IMChatInputText;
                if (aloneText != null)
                {
                ...
                if (tx + aloneItem.Width + IMChatContentEmotionAndText.RightPadding >= 310)
                {
                    tx = 0;
                    ty += RowHeight;
                }
        ...
        public void Draw(Graphics g)
        {
            ...
            for (int i = 0; i < AllTestAndEmotion.Count; i++)
            {
                ...
                if (widtha + aloneItem.Width + IMChatContentEmotionAndText.RightPadding + 4 >= ChatConstraint)
                {
                    counta++;
                    widtha = 0;
                    heighta += RowHeight;
            ...
            for (int i = 0; i < listotal; i++)
            {
                IIMChatInputItem aloneItem = AllTestAndEmotion[i];
                IMChatInputText aloneText = aloneItem as IMChatInputText;
                //IMChatInPutImage aloneImage= aloneItem as IMChatInPutImage;
                ...
                if (aloneX + aloneItem.Width + IMChatContentEmotionAndText.RightPadding + 4 >= ChatConstraint + (int)rectPos.X)
                {
                    aloneX = (int)rectPos.X;
                    aloneY += RowHeight;
                }
    ...
    //发送图片
    internal class IMChatContentImage : IIMChatControl
    {
        private int offsetY;
        private bool Isme;
        private IChatControl parent;
        ...
        public IMChatContentImage(Image _messageImage, IChatControl _parent, SenderInfo senderInfo)
        {
    ...
    //发送文件
    internal class IMFileBox : IIMChatControl
    {
        ...
        /// <summary>
        /// FileCanceled 当点击“取消”按钮时，将触发此事件。参数： IMFileBox, projectID,isSend
        /// </summary>
        public event System.Action<IMFileBox, string, bool> FileCanceled;
        /// <summary>
        /// 当 点击 “接收”按钮 时，触发  参数：IMFileBox,projectID,isSend,savePath
        /// </summary>
        public event System.Action<IMFileBox, string, bool, string> FileReceived;
        /// <summary>
        ...
        /// 当点击“转离线发送”按钮时，触发。
        /// </summary>
        public event CbGeneric<TransferingProject, IMFileBox> ConvertToOfflineFile;
        ...
        public IMFileBox(IChatControl _parent)
        {
            this.parent = _parent;
        ...
        public void Draw(Graphics g)
        {
            ...
            //通过判断是谁发送，确认发送者头像和IMFileBox控件的位置
            if (Isme)
            {
                if (disNameIf)
                {
        ...
        private void linkLabel_receive_LinkClicked()
        {
            ...
            {
                MessageBox.Show(ee.Message, "IMFileBox.linkLabel_receive_LinkClicked");
            }
        }
        ...
        private void skinLabel_Cancel_Click()
        {
            ...
            {
                MessageBox.Show(ee.Message, "IMFileBox.skinLabel_Cancel_Click");
            }
        }
        ...
        private void skinLabel_Open_Click()
        {
            ...
            {
                MessageBox.Show(ee.Message, "IMFileBox.skinLabel_Open_Click");
            }
        }
    }

    internal class IMChatCard : IIMChatControl
    {
        #region 参数
        ...
        public IMChatCard(PersonCardModel card, ref int offsetY, IChatControl parent, SenderInfo senderInfo)
        {
    ...
    internal class IMAudioMessageBox : IIMChatControl
    {
        public IChatControl parent { get; set; }
        public int offsetY { get; set; }
        private int height;
        ...
        public IMAudioMessageBox(AudioMessage msg, ref int _offsetY, IChatControl _parent, SenderInfo senderInfo)
        {
            this.isMe = senderInfo.IsMe;
            this.headImage = senderInfo.HeadImg;
            this.sendName = senderInfo.DisplayName;
    ...
    public interface IIMChatInputItem
    {
    ...
    internal class IMChatInputText : IIMChatInputItem
    {
        private DrawTextAttribute drawTextAttribute;
        private Brush brush;
        private Size size;

        public IMChatInputText(DrawTextAttribute _drawTextAttribute, Brush brush)
        {
    ...
    internal class IMChatInPutEmotion : IIMChatInputItem
    {
        ...
        public IMChatInPutEmotion(int _emotionIndex, Image bitmap)
        {
            this.image = bitmap;
    ...
    internal class IMChatInPutImage : IIMChatInputItem
    {
        private Image bitmap;
        private int emotionIndex;
        private Image image;
        ...
        public IMChatInPutImage(Image bitmap)
        {
            this.bitmap = bitmap;
        }
        public string Content
