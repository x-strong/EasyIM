...
namespace EasyIM.Controls
{
    public partial class ChatInPutBox : UserControl
    {
        ...
        private List<IIMChatInputItem> inputItemListRebuild = new List<IIMChatInputItem>();
        ...
        protected override void OnPaint(PaintEventArgs e) 
        {
            ...
            for (int i = 0; i < this.inputItemListRebuild.Count; i++)
            {
                ...
                IIMChatInputItem item = this.inputItemListRebuild[i];
                IMChatInputText textItem = item as IMChatInputText;
        ...
        private void AppendText(String _key_str)
        {
            ...
            IMChatInputText item = new IMChatInputText(drawTextAttribute, new SolidBrush(Color.Black));
            this.InsertInputItem(item);
        }

        public void AppendEmotion(int _emotionIndex, Bitmap bitmap)
        {
            this.InsertInputItem(new IMChatInPutEmotion(_emotionIndex, bitmap));
            this.Focus();
        }
        ...
        private void InsertInputItem(IIMChatInputItem item)
        {
            if (this.cursorIndex != this.inputItemListRebuild.Count)
            {
        ...
        internal IMInputContent GetInputContent()
        {
            ...
            {
                IIMChatInputItem item = this.inputItemListRebuild[i];
                IMChatInputText text = item as IMChatInputText;
                IMChatInPutEmotion emotion = item as IMChatInPutEmotion;
                IMChatInPutImage image = item as IMChatInPutImage;
                if (text != null)
                {
            ...
            return new IMInputContent() { EmotionDictionary = emotionDictionary, ImageDictionary = imageDictionary, Text = sb.ToString() };
        }
        ...
        public void OnPointerCursor(Point p)
        {
            ...
            for (int i = 0; i < this.inputItemListRebuild.Count; i++)
            {
                IIMChatInputItem pointerListItem = inputItemListRebuild[i];
