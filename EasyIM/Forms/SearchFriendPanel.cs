...
namespace EasyIM
{
    ...
    {       
        ...
        private IChatSupporter supporter;
        public SearchFriendPanel()
        {
            InitializeComponent();                    
        }

        public void Initialize( ResourceCenter<IMUser, IMGroup > center,IChatSupporter supporter)
        {
        ...
        private void skinTextBox_id_SkinTxt_TextChanged(object sender, EventArgs e)
        {
            ...
            List<IMUser> users = this.resourceCenter.ClientGlobalCache.SearchUser_Fuzzy(id);
            List<IMGroup> groups = this.resourceCenter.ClientGlobalCache.SearchGroup_Fuzzy(id);
            this.skinLabel_noResult.Text = "按回车获取结果";
            List<IGroup> iGroups = new List<IGroup>();
            if (groups != null)
            {
                iGroups = ESBasic.Collections.CollectionConverter.ConvertListUpper<IGroup, IMGroup>(groups);
            }
            
            this.SetSearchResult(users, iGroups);
        }

        //设置查询结果
        private void SetSearchResult(List<IMUser> users, List<IGroup> groups,bool isFromServer=false)
        {
            ...
            if (!hasNoResult)
            {
                ...
                {
                    Image headImage = global::EasyIM.Properties.Resources.Group2;
        ...
        private void chatListBox_DoubleClickSubItem(object sender, ChatListEventArgs e)
        {
            ...
            {
                this.StartUserChat((IMUser)unit);
            }
            else if (unit.UnitType == UnitType.Group)
            {
                this.StartGroupChat((IMGroup)unit);
            }            
        }

        private void StartGroupChat(IMGroup group)
        {
            if (group == null)
            {
                return;
        ...
        private void StartUserChat(IMUser friend)
        {
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            {
                GroupInfoForm form = new GroupInfoForm(this.resourceCenter, (IMGroup)unit);
                form.Show();
            }
        ...
        {
            IMUser friend = (IMUser)this.chatListBox.SelectSubItem.Tag;
