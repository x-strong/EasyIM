...
namespace EasyIM.Core
{
    internal class ChatBoxCore: IFileTransferingViewer
    {
        ...
        private List<IIMChatControl> controlList = new List<IIMChatControl>();
        ...
        private void AppendSendTime(DateTime time)
        {
            lock (this.locker)
            {
                if (time.Subtract(this.lastMessageTime) > TimeSpan.FromMinutes(5))
                {
                    ...
                    IMSystemTipLabel o = new IMSystemTipLabel(datea, this.parent);
        ...
        {
            IMSystemTipLabel addSystemremind = new IMSystemTipLabel(msg, this.parent);
        ...
        {
            List<IIMChatInputItem> items = this.GetIIMChatInputItems4Text(content);
            List<IMChatInPutImage> images = this.GetIMChatInPutImages(content);
            ...
            foreach (IMChatInPutImage item in images)
            {
        ...
        private List<IIMChatInputItem> GetIIMChatInputItems4Text(ESFramework.Boost.Controls.ChatBoxContent content)
        {
            if (content == null || content.Text == null)
            {
                return null;
            }
            List<IIMChatInputItem> inputItemListRebuild = new List<IIMChatInputItem>();
            ...
            if (content.NonTextItemDictionary != null && content.NonTextItemDictionary.Count > 0)
            {
                ...
                {
                    inputItemListRebuild.Add(this.GetIMChatInputText(item.ToString()));
                }
                ...
                for (int i = 0; i < content.NonTextItemPositions.Count; i++)
                {
                    ...
                    {
                        inputItemListRebuild.Insert((int)position, new IMChatInPutEmotion((int)item.InnerEmotionIndex, GlobalResourceManager.EmotionDictionary[item.InnerEmotionIndex]));
                    }
                    else if (item.ChatBoxElementType == ChatBoxElementType.ForeignImage)
                    {
                        //inputItemListRebuild.Insert((int)position, new IMChatInPutImage(item.ForeignImage));
        ...
        private IMChatInputText GetIMChatInputText(string chars)
        {
            DrawTextAttribute inputText = this.CreateFormattedText(chars);
            return new IMChatInputText(inputText, inputTextBrush);
        }
        ...
        private List<IMChatInPutImage> GetIMChatInPutImages(ESFramework.Boost.Controls.ChatBoxContent content)
        {
            if (content == null)
            {
                return null;
            }
            List<IMChatInPutImage> chatInPutImages = new List<IMChatInPutImage>();
            if (content.NonTextItemDictionary != null && content.NonTextItemDictionary.Count > 0)
            {
                //插入NonTextItem
                for (int i = 0; i < content.NonTextItemPositions.Count; i++)
                {
                    ...
                    {
                        chatInPutImages.Add(new IMChatInPutImage(item.ForeignImage));
                    }

                }
            }
            return chatInPutImages;
        }
        #endregion
        ...
        private void AppendTextAndEmoji(List<IIMChatInputItem> all, SenderInfo senderInfo, DateTime time)
        {
            this.AppendSendTime(time);
            senderInfo.SenderShowType = this.SenderShowType;
            controlList.Add(new IMChatContentEmotionAndText(all, this.parent, senderInfo));
            this.AutoSizeChanged();
        }
        ...
        private void AppendImage(Image _messageImage, SenderInfo senderInfo, DateTime time)
        {
            this.AppendSendTime(time);
            senderInfo.SenderShowType = this.SenderShowType;
            controlList.Add(new IMChatContentImage(_messageImage, this.parent, senderInfo));
            this.AutoSizeChanged();
        }
        ...
        private void AppendCard(PersonCardModel cardModel, SenderInfo senderInfo, DateTime time)
        {
            this.AppendSendTime(time);
            senderInfo.SenderShowType = this.SenderShowType;
            controlList.Add(new IMChatCard(cardModel, ref offsetY, this.parent, senderInfo));
            this.AutoSizeChanged();
        }
        ...
        private void AppendAudioMessageBox(SenderInfo senderInfo, AudioMessage msg, DateTime time)
        {
            ...
            this.AppendSendTime(time);
            senderInfo.SenderShowType = this.SenderShowType;
            IMAudioMessageBox audioMessageBox = new IMAudioMessageBox(msg, ref offsetY, this.parent, senderInfo);
            controlList.Add(audioMessageBox);
            this.audioMessageBoxDic.Add(msg.GetUniqueID(), audioMessageBox);
            this.AutoSizeChanged();
        }
        #endregion
        ...
        public void AutoSizeChanged()
        {
            lock (this.locker2)
            {
                ...
                for (int i = 0; i < this.controlList.Count; i++)
                {
                    ...
                    if (this.controlList[i] is IMChatContentEmotionAndText)
                    {
                        this.totalHeight += 10;
                    }
        ...
        private IIMChatControl lastOnEnterControl = null;
        public void OnMouseMove(Point location)
        {
            for (int i = 0; i < this.controlList.Count; i++)
            {
                ...
                {
                    if (this.lastOnEnterControl == controlList[i])//若在同一控件上移动,不再次触发OnMouseMove事件 （IMFileBox 除外）
                    {
                        IMFileBox fileBox = controlList[i] as IMFileBox;
                        if (fileBox != null)
                        {
                            controlList[i].OnMouseMove(location);
                        }

                        IMAudioMessageBox audioMessageBox = controlList[i] as IMAudioMessageBox;
        ...
        public void OnMouseDown(Point location)
        {
            for (int i = 0; i < this.controlList.Count; i++)
            {
                ...
                if (fileBox != null)
                {
                    if (fileBox.AllSize.Contains(location))
                    {
                ...
                if (audioMessageBox != null)
                {
                    if (audioMessageBox.AllSize.Contains(location))
                    {
                        controlList[i].OnMouseDown(location);
                ...
                IMChatContentImage chatContentImage = controlList[i] as IMChatContentImage;
                if (chatContentImage != null)
                {
                    if (chatContentImage.AllSize.Contains(location))
                    {
                        controlList[i].OnMouseDown(location);
                        this.ImageMessageClicked(chatContentImage.MessageImage);
                        return;
                    }
                    continue;
                }
        ...
        private Dictionary<string, IMAudioMessageBox> audioMessageBoxDic = new Dictionary<string, IMAudioMessageBox>();
        ...
        private ObjectManager<string, IMFileBox> fileBoxManager = new ObjectManager<string, IMFileBox>();
        ...
        #region Initialize
        public void Initialize(ResourceCenter<IMUser, IMGroup> _resourceCenter, IFileOutter _fileOutter)
        {
            this.Initialize(_resourceCenter, _fileOutter, (ESBasic.Func<TransferingProject, bool>)null);
        }

        public void Initialize(ResourceCenter<IMUser, IMGroup> _resourceCenter, IFileOutter _fileOutter, string friendUserID)
        {
        ...
        public void Initialize(ResourceCenter<IMUser, IMGroup> _resourceCenter, IFileOutter _fileOutter, ESBasic.Func<TransferingProject, bool> filter)
        {
        ...
        public List<string> GetTransferingProjectIDsInCurrentViewer()
        {
            ...
            List<IMFileBox> boxes = this.fileBoxManager.GetAllReadonly();
            foreach (IMFileBox item in boxes)
            {
        ...
        private void dofileReceiverManager_FileResumedTransStarted(TransferingProject info)
        {
            ...
            IMFileBox item = this.GetExistedItem(info.ProjectID);
        ...
        {
            IMFileBox item = this.GetExistedItem(projectID);
            if (item != null)
            {
        ...
        private void AddFileTransItem(TransferingProject project, bool offlineFile, bool doneAgreed)
        {
            ...
            IMFileBox fileBox = this.GetExistedItem(project.ProjectID);
            if (fileBox != null)
            {
                return;
            }
            fileBox = new IMFileBox(this.parent);
            fileBox.FileCanceled += fileTransItem_FileCanceled;
            fileBox.FileReceived += fileTransItem_FileReceived;
            fileBox.FileRejected += fileTransItem_FileRejected;
            fileBox.ConvertToOfflineFile += new CbGeneric<TransferingProject, IMFileBox>(fileTransItem_ConvertToOfflineFile);
            ...
            IMUser sender = this.resourceCenter.ClientGlobalCache.GetUser(CommonHelper.GetUserID4OMCS(senderID));
            senderInfo.HeadImg = GlobalResourceManager.GetHeadImage(sender);
            senderInfo.DisplayName = sender.DisplayName;
            senderInfo.SenderShowType = this.SenderShowType;
            fileBox.Initialize(project, offlineFile, doneAgreed, senderInfo);
            this.fileBoxManager.Add(fileBox.TransferingProject.ProjectID, fileBox);
            this.controlList.Add(fileBox);
            this.FileTransStarted(project.ProjectName, project.IsSender);
            this.AutoSizeChanged();
            this.ContentScrollToEnd();
        }

        void fileTransItem_ConvertToOfflineFile(TransferingProject project, IMFileBox item)
        {
        ...
        private IMFileBox GetExistedItem(string projectID)
        {
        ...
        void fileTransItem_FileReceived(IMFileBox item, string projectID, bool isSend, string savePath)
        {
            this.fileOutter.BeginReceiveFile(projectID, savePath);
        }

        void fileTransItem_FileCanceled(IMFileBox item, string projectID, bool isSend)
        {
