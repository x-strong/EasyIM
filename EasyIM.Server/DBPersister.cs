...
{   
    public interface IDBPersisterExtend: IDBPersister<IMUser, IMGroup>
    {
        /// <summary>
        /// 根据用户ID获取其手机号
        /// </summary>
    ...
    public class DBPersister : DefaultChatRecordPersister, IDBPersisterExtend
    {
        ...
        public void InsertUser(IMUser t)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                accesser.Insert(t);
                scope.Commit();
            }
        }
        ...
        public void InsertGroup(IMGroup t)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMGroup> accesser = scope.NewOrmAccesser<IMGroup>();
                accesser.Insert(t);
                scope.Commit();
            }
        }
        ...
        public void UpdateUserPcOfflineTime(string userID,DateTime dateTime)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                accesser.Update(new ColumnUpdating(IMUser._PcOfflineTime, dateTime),new Filter(IMUser._UserID,userID));
                scope.Commit();
            }
        }
        ...
        public void UpdateUserMobileOfflineTime(string userID,DateTime dateTime)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                accesser.Update(new ColumnUpdating(IMUser._MobileOfflineTime, dateTime), new Filter(IMUser._UserID, userID));
                scope.Commit();
            }
        }
        ...
        public void UpdateUserFriends(IMUser t)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                ...
                list.Add(new ColumnUpdating(IMUser._Friends, t.Friends));
                list.Add(new ColumnUpdating(IMUser._Version, t.Version));
                accesser.Update(list, new Filter(IMUser._UserID, t.UserID));
                scope.Commit();
            }
        }
        ...
        {
            List<IMUser> list = new List<IMUser>();
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
        ...
        public List<IMGroup> SearchGroup(string idOrName)
        {
            ...
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                ...
                IFilterTree filterTree = new SimpleFilterTree(LogicType.Or, new Filter(IMGroup._GroupID, id), new Filter(IMGroup._Name, idOrName));
        ...
        public void UpdateUserPassword(string userID, string newPasswordMD5)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                accesser.Update(new ColumnUpdating(IMUser._PasswordMD5, newPasswordMD5), new Filter(IMUser._UserID, userID));
                scope.Commit();
            }
        }
        ...
        public void ChangeUserState(string userID, UserState userState, int version)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                ...
                columnUpdatings.Add(new ColumnUpdating(IMUser._UserState, userState));
                columnUpdatings.Add(new ColumnUpdating(IMUser._Version, version));
                accesser.Update(columnUpdatings, new Filter(IMUser._UserID, userID));
                scope.Commit();
            }
        }

        public void UpdateUserGroups(IMUser user)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                List<ColumnUpdating> list = new List<ColumnUpdating>();
                list.Add(new ColumnUpdating(IMUser._Groups, user.Groups));
                list.Add(new ColumnUpdating(IMUser._Version, user.Version));
                accesser.Update(list, new Filter(IMUser._UserID, user.ID));
                scope.Commit();
            }
        }

        public void UpdateGroupMembers(IMGroup group)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMGroup> accesser = scope.NewOrmAccesser<IMGroup>();
                List<ColumnUpdating> list = new List<ColumnUpdating>();
                list.Add(new ColumnUpdating(IMGroup._Members, group.Members));
                list.Add(new ColumnUpdating(IMGroup._Version, group.Version));
                accesser.Update(list, new Filter(IMGroup._GroupID, group.ID));
                scope.Commit();
            }
        }
        ...
        {
            List<IMUser> userList = new List<IMUser>();
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                userList = accesser.GetMuch(new SimpleFilterTree(LogicType.Or,new Filter(IMUser._UserID, idOrName),new Filter(IMUser._Name,idOrName))) ;
                scope.Commit();
            }
            return userList;
        }
        ...
        public IMUser GetUser4Phone(string phone)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                user = accesser.GetOne(new Filter(IMUser._Phone,phone));
                scope.Commit();
            }
            return user;
        }
        ...
        public List<IMUser> GetUserList4Phone(string phone)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                userList = accesser.GetMuch(new Filter(IMUser._Phone, phone));
                scope.Commit();
            }
            return userList;
        }
        ...
        public int GetUserCount4Phone(string phone)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                userCount = (int)accesser.GetCount(new Filter(IMUser._Phone, phone));
                scope.Commit();
            }
            return userCount;
        }
        ...
        public string GetPhone4UserID(string userID)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                phone = (string)accesser.GetColumnValue(userID,IMUser._Phone);
                scope.Commit();
            }
            return phone;
        }
        ...
        public string GetUserPassword(string userID)
        {
            ...
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                pwd = accesser.GetColumnValue(userID, IMUser._PasswordMD5);
                scope.Commit();
            }
            if (pwd == null)
            {
        ...
        public void UpdateUserInfo(string userID, string name, string signature, string orgID, int version)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                List<ColumnUpdating> list = new List<ColumnUpdating>();
                list.Add(new ColumnUpdating(IMUser._Signature, signature));
                list.Add(new ColumnUpdating(IMUser._Version, version));
                list.Add(new ColumnUpdating(IMUser._Name, name));
                list.Add(new ColumnUpdating(IMUser._OrgID, orgID));
                accesser.Update(list, new Filter(IMUser._UserID, userID));
                scope.Commit();
            }
        }
        ...
        public void UpdateUserHeadImage(string userID, int defaultHeadImageIndex, byte[] customizedHeadImage, int version)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                List<ColumnUpdating> list = new List<ColumnUpdating>();
                list.Add(new ColumnUpdating(IMUser._HeadImageIndex, defaultHeadImageIndex));
                list.Add(new ColumnUpdating(IMUser._HeadImageData, customizedHeadImage));
                list.Add(new ColumnUpdating(IMUser._Version, version));
                accesser.Update(list, new Filter(IMUser._UserID, userID));
                scope.Commit();
            }
        ...
        public void UpdateUserPhone(string userID, string phone, int version)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                List<ColumnUpdating> list = new List<ColumnUpdating>();
                list.Add(new ColumnUpdating(IMUser._Phone, phone));
                list.Add(new ColumnUpdating(IMUser._Version, version));
                accesser.Update(list, new Filter(IMUser._UserID, userID));
                scope.Commit();
            }
        ...
        public void UpdateUserCommentNames(IMUser t)
        {
            using (TransactionScope scope = this.transactionScopeFactory.NewTransactionScope())
            {
                IOrmAccesser<IMUser> accesser = scope.NewOrmAccesser<IMUser>();
                List<ColumnUpdating> list = new List<ColumnUpdating>();
                list.Add(new ColumnUpdating(IMUser._CommentNames, t.CommentNames));
                list.Add(new ColumnUpdating(IMUser._Version, t.Version));
                accesser.Update(list, new Filter(IMUser._UserID, t.UserID));
                scope.Commit();
            }
    ...
    public class DBPersister_SqlSugar : DefaultChatRecordPersister2, IDBPersisterExtend
    {
        ...
        public void DeleteUser(string userID)
        {
            db.Deleteable<IMUser>().Where(ou => ou.UserID == userID).ExecuteCommand();
        }
        ...
        public void DeleteGroup(string groupID)
        {
            db.Deleteable<IMGroup>().Where(og => og.GroupID == groupID).ExecuteCommand();
        }
        ...
        public List<IMUser> GetAllUser()
        {
            return db.Queryable<IMUser>().ToList();
        }

        public List<IMGroup> SearchGroup(string idOrName)
        {
            string id = idOrName.StartsWith(FunctionOptions.PrefixGroupID) ? idOrName : FunctionOptions.PrefixGroupID + idOrName;
            return db.Queryable<IMGroup>().Where(it => it.GroupID == id || it.Name.Contains(idOrName)).ToList();
        }

        public List<IMGroup> GetAllGroup()
        {
            return db.Queryable<IMGroup>().ToList();
        }

        public void UpdateUserPassword(string userID, string newPasswordMD5)
        {
            db.Updateable<IMUser>(it => it.PasswordMD5 == newPasswordMD5).Where(it => it.UserID == userID).ExecuteCommand();
        }

        public void ChangeUserState(string userID, UserState userState, int version)
        {
            db.Updateable<IMUser>(it => new IMUser() { UserState = userState, Version = version }).Where(it => it.UserID == userID).ExecuteCommand();
        }
        ...
        {
            IMUser user = null;
            user = db.Queryable<IMUser>().Where(it => it.UserID == userID).First();
            return user;
        }

        public List<IMUser> SearchUser(string idOrName)
        {
            return db.Queryable<IMUser>().Where(it => it.UserID == idOrName || it.Name.Contains(idOrName)).ToList();
        }

        public IMUser GetUser4Phone(string phone)
        {
            IMUser user = null;
            user = db.Queryable<IMUser>().Where(it => it.Phone == phone).First();
            return user;
        }
        ...
        {
            string phone = string.Empty;
            phone = db.Queryable<IMUser>().Where(it => it.UserID == userID).Select(it => it.Phone).First();
            return phone;
        }

        public List<IMUser> GetUserList4Phone(string phone)
        {
            return db.Queryable<IMUser>().Where(it => it.Phone == phone).ToList();
        }
        ...
        {
            object pwd = null;
            pwd = db.Queryable<IMUser>().Where(it => it.UserID == userID).Select(it => it.PasswordMD5).First();
        ...
        {
            IMGroup group = null;
            group = db.Queryable<IMGroup>().Where(it => it.GroupID == groupID).First();
            return group;
        }

        public void UpdateUserInfo(string userID, string name, string signature, string orgID, int version)
        {
            db.Updateable<IMUser>(it => new IMUser() { Signature = signature, Name = name, OrgID = orgID, Version = version }).Where(it => it.UserID == userID).ExecuteCommand();
        }

        public void UpdateUserHeadImage(string userID, int defaultHeadImageIndex, byte[] customizedHeadImage, int version)
        {
            db.Updateable<IMUser>(it => new IMUser() { HeadImageIndex = defaultHeadImageIndex, HeadImageData = customizedHeadImage, Version = version }).Where(it => it.UserID == userID).ExecuteCommand();
        }
        ...
        public void UpdateUserPcOfflineTime(string userID, DateTime dateTime)
        {
            db.Updateable<IMUser>(it => it.PcOfflineTime == dateTime).Where(it => it.UserID == userID).ExecuteCommand();
        }
        ...
        public void UpdateUserMobileOfflineTime(string userID, DateTime dateTime)
        {
            db.Updateable<IMUser>(it => it.MobileOfflineTime == dateTime).Where(it => it.UserID == userID).ExecuteCommand();
        }

        public void UpdateUserPhone(string userID, string phone, int version)
        {
            db.Updateable<IMUser>(it => new IMUser { Phone = phone, Version = version }).Where(it => it.UserID == userID).ExecuteCommand();
        }
